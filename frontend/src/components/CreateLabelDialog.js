...
export default function CreateLabelDialog({ open, onClose, onLabelCreated, onBuildFromInstructions }) {
  ...
  async function handleSave() {
    if (useInstructions) {
      // Use instructions to rebuild automations
    ...
    <div className="dialog-backdrop">
      <div className="dialog">
        ...
        <div style={{margin:'10px 0'}}>
          <input type="checkbox" checked={useInstructions} onChange={e => setUseInstructions(e.target.checked)} />
        ...
        {useInstructions ? (
          <>
            ...
            <textarea value={instructions} onChange={e => setInstructions(e.target.value)} style={{width:'100%',height:'100px'}} />
          </>
        ) : (
          <>
